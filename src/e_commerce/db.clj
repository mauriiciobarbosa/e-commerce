...
(s/defn todos-os-produtos-da-categoria :- [model/Produto]
  [db categorias :- [s/Str]]
  (datomic-para-entidade
    (d/q '[:find [(pull ?produto [* {:produto/categoria [*]}]) ...]
...
(s/defn todos-os-produtos-da-categoria-e-digital :- [model/Produto]
  [db
   categorias :- [s/Str]
   digital? :- s/Bool]
  (datomic-para-entidade
...
(defn change-email
  ...
  (let [db (d/db conn)
        entityId (d/q '[:find ?e .
                      ...
                      db "bob@example.com")]
    (d/transact conn [{:db/id entityId :person/name "Robert"}])))

(defn change-email-using-lookup-ref
  [conn]
  (d/transact conn [{:db/id [:person/email "bob@example.com"] :person/name "Robert"}]))
...
(s/defn total-de-produtos :- s/Int
  [db]
  (d/q '[:find (count ?p) .
         :where [?p :produto/id]]
       db))
...
;(s/defn ^:private visualizacao :- s/Int
...
;  [conn produto-id :- s/Uuid]
;  (let [numero-de-visualizacoes (visualizacao (d/db conn) produto-id)
;        inc-num-visualizacoes (inc numero-de-visualizacoes)]
;    (d/transact conn [{:produto/id produto-id
;                       :produto/visualizacoes inc-num-visualizacoes}])))
